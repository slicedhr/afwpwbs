...
let getEmpresa = (id) => {
  ...
  return R.connect(Config.db).then(conn => {

    let query = R.table(ENTITY);

    return id ? query.get(id).run(conn) : query.orderBy(R.desc('created_at')).run(conn).then(cursor => cursor.toArray());
...
export default class Empresa {
  ...
  return getEmpresa(id).then(data => res.send(data || { empty: true }));
